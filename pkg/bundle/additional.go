...
func GetAdditional(cfg *BundleSpec, rootDir string) error {
	...
	for _, img := range cfg.Mirror.AdditionalImages {
		...
		if IsBlocked(cfg, srcRef.Ref) {
			return fmt.Errorf("additional image %s also specified as blocked, remove the image one config field or the other", img)
		}
