...
func (o LocalStorageCollector) buildMirrorToDiskPaths(img v2alpha1.Image, imgSpec image.ImageSpec, targetRepo, targetTag string) (string, string) {
	...
	if imgSpec.Transport != dockerProtocol {
		...
		tmpDest = strings.Join([]string{o.destinationRegistry(), strings.TrimPrefix(targetRepo, "/")}, "/") + ":" + tag
...
func (o LocalStorageCollector) buildDiskToMirrorPaths(img v2alpha1.Image, imgSpec image.ImageSpec, targetRepo, targetTag string) (string, string) {
	if imgSpec.Transport != dockerProtocol {
		...
		tmpSrc := strings.Join([]string{o.LocalStorageFQDN, strings.TrimPrefix(targetRepo, "/")}, "/") + ":" + tag
		tmpDest := strings.Join([]string{o.Opts.Destination, strings.TrimPrefix(targetRepo, "/")}, "/") + ":" + tag
		return tmpSrc, tmpDest
	}
	...
	case imgSpec.IsImageByDigestOnly() && img.TargetTag != "":
		tmpSrc = strings.Join([]string{o.LocalStorageFQDN, targetRepo + ":" + targetTag}, "/")
		tmpDest = strings.Join([]string{o.Opts.Destination, targetRepo + ":" + targetTag}, "/")
	case imgSpec.IsImageByDigestOnly() && o.generateV1DestTags:
	...
	case imgSpec.IsImageByDigestOnly():
		...
		tmpSrc = strings.Join([]string{o.LocalStorageFQDN, targetRepo + ":" + digestTag}, "/")
		tmpDest = strings.Join([]string{o.Opts.Destination, targetRepo + ":" + digestTag}, "/")
	default:
		tmpSrc = strings.Join([]string{o.LocalStorageFQDN, targetRepo}, "/") + ":" + targetTag
		tmpDest = strings.Join([]string{o.Opts.Destination, targetRepo}, "/") + ":" + targetTag
	}
