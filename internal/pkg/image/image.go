...
// (e.g. "sha256-abc..."), or "" if the image has no digest. The result is
// truncated to 128 characters to comply with the max tag length enforced by
// the OCI image spec / container registries.
func (i ImageSpec) AlgorithmAndDigest() string {
	if i.Digest == "" {
		return ""
	}
	tag := i.Algorithm + "-" + i.Digest
	if len(tag) > 128 {
		tag = tag[:127]
	}
	return tag
