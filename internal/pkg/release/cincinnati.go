...
func (o *CincinnatiSchema) GetReleaseReferenceImages(ctx context.Context) ([]v2alpha1.CopyImageSchema, error) {
	...
	if len(filterCopy.Platforms) > 0 {
		...
		archs := filterCopy.Architectures
		if len(archs) > 0 {
			o.Log.Warn("platform.architectures is deprecated; use platform.platforms instead for sparse manifest platform filtering")
		} else {
			// Neither Platforms nor Architectures was set — default to amd64 for backward compat.
			archs = []string{v2alpha1.DefaultPlatformArchitecture}
		}
...
func (o *CincinnatiSchema) collectChannelImages(ctx context.Context, channels []v2alpha1.ReleaseChannel, flagReport *bool) ([]v2alpha1.CopyImageSchema, []error) {
	...
	for _, ch := range channels {
		...
		*flagReport = o.warnEUSChannel(ch, *flagReport)
...
// It returns true if the warning was printed, or the value of alreadyReported if the warning
// was skipped (either because it was already printed or the channel does not qualify).
func (o *CincinnatiSchema) warnEUSChannel(ch v2alpha1.ReleaseChannel, alreadyReported bool) bool {
	if len(ch.MaxVersion) == 0 || len(ch.MinVersion) == 0 || alreadyReported {
