...
func TestExecutorSetupLocalRegistryConfig(t *testing.T) {
	// The local storage registry is only consumed in-process, so it is expected to
	// stay on the loopback interface: an empty or wildcard bind host would put it on
	...
	t.Run("Testing Executor : local registry binds to loopback only", func(t *testing.T) {
		...
		ip := net.ParseIP(host)
		assert.NotNil(t, ip, "bind host should be a literal IP, not a name that could resolve off-loopback")
		assert.True(t, ip.IsLoopback(), "local storage registry should bind a loopback address, got %q", host)
	})
}
