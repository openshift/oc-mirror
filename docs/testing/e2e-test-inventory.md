...
| 3 | 73452 | Mirror2mirror for OCI operator and additional image | m2m for OCI operator catalog, creates CatalogSource, installs operator from custom CS | E2E (migrate core to Integration) | High | | Distinctive oc-mirror functionality (oci:// local catalog input, targetCatalog/targetTag, additional images, cluster-resource generation) does NOT need a cluster and should move to Integration with a small self-built catalog. Only the operator-install/pod-Running check needs a cluster, and that path is already covered by 73359 and 79215. `[Level0]` tag removed. |
...
| 16 | 72971 | Mirror multiple catalogs (docker + OCI) | m2m with multiple catalog sources (certified, marketplace, OCI), installs operator | E2E (migrate core to Integration) | High | | Distinctive oc-mirror functionality (multiple/mixed-source catalogs — one oci:// local + docker:// — in a single ISC, one CatalogSource generated per catalog) does NOT need a cluster and should move to Integration with small self-built catalogs. Only the operator-install/pod-Running check needs a cluster, and that path is already covered by 73359 and 79215 (two of the three installs here are already commented out). `[Level0]` tag removed. |
