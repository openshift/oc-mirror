...
import (
	...
	mockmanifest "github.com/openshift/oc-mirror/v2/internal/pkg/manifest/mock"
...
func TestReleaseLocalStoredCollector(t *testing.T) {
	...
	t.Run("Testing ReleaseImageCollector - Mirror to disk: should pass", func(t *testing.T) {
		manifestMock := mockmanifest.NewMockManifestInterface(mockCtrl)
	...
	t.Run("Testing ReleaseImageCollector : should fail image index", func(t *testing.T) {
		mockManifest := mockmanifest.NewMockManifestInterface(mockCtrl)
...
func TestHandleGraphImage(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			manifestMock.
				EXPECT().
				ImageDigest(gomock.Any(), gomock.Any(), gomock.Eq("oci://"+copyOpts.Global.WorkingDir+"/"+graphPreparationDir)).
				DoAndReturn(func(any, any, any) (string, error) {
