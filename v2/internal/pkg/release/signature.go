	...
	for _, img := range images {
		...
		if digest != "" {
			...
			if data = o.getCachedSignature(digest); len(data) == 0 {
				// we dont have the current digest in the cache, do a lookup and download it
		...
		// OCPBUGS-52562: ensure not duplicates
...
func (o SignatureSchema) downloadSignature(ctx context.Context, client *http.Client, signatureURL string, digest string) ([]byte, error) {
	req, _ := http.NewRequest("GET", signatureURL+"sha256="+digest+"/signature-1", nil) //nolint:gosec // FIXME: G704: SSRF via taint analysis
...
func (o SignatureSchema) verifySignature(algorithm string, digest string, data []byte) (string, error) {
	...
	// NOTE: md.SignatureError cannot be trusted until md.UnverifiedBody has
	// been read to EOF: per the opengpg API, the signature is verified as the
	// body is consumed, and the check can only complete once the whole message
	// has been read. So we must fully read UnverifiedBody first, then check
	// SignatureError.
	...
	if err := json.Unmarshal(content, &signSchema); err != nil {
...
func (o SignatureSchema) saveSignature(imgRef string, digest string, data []byte) error {
	...
	// newImgSpec.Tag comes from the (already cryptographically verified)
	// signed payload's docker-reference. image.ParseRef does not validate it
	// against the Docker tag grammar, so it can contain path separators or
	// ".." sequences (e.g. a docker-reference of
	// "repo:evil/../../../etc/passwd"). filepath.Base collapses that down to a
	// single path element, which fully neutralizes any attempt to escape the
	// signature cache directory below (gosec: path traversal via taint).
	...
	if _, err := os.Stat(sigFilePath); err != nil { //#nosec G703 // path is sanitized
		if ferr := os.WriteFile(sigFilePath, data, 0o600); ferr != nil { //#nosec G703 // path is sanitized
			return fmt.Errorf("write signature: %w", ferr)
		}
	}
