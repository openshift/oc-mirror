...
// TestVerifySignature exercises SignatureSchema.verifySignature in isolation,
// now that signature verification is its own function independent of
// caching/HTTP concerns. It uses a throwaway PGP identity generated at test
// time, so none of these cases depend on network access or the real OCP
// signing key.
func TestVerifySignature(t *testing.T) {
	...
	// sign returns an OpenPGP-signed message wrapping the JSON encoding of
	// content, matching the format of a real "atomic container signature".
	...
	t.Run("tampered signature is rejected", func(t *testing.T) {
		// This guards against a regression where
		// openpgp.MessageDetails.SignatureError was checked immediately after
		// openpgp.ReadMessage() returned, before md.UnverifiedBody had been
		// read: per the openpgp API, the signature can only be verified once
		// the body has been fully consumed.
		data := sign(t, validContent)

		// Tamper with the very last byte of the message: the trailing
		// Signature packet is always serialized last (after the literal data
		// content, regardless of any chunking), so this corrupts only the
		// signature's cryptographic material while leaving the signed JSON
		// payload completely intact and parseable.
	...
	t.Run("mismatched digest is rejected", func(t *testing.T) {
		// The payload is validly signed, but it attests to a different image
		// digest than the one we're asking to verify.
...
// TestSaveSignatureSanitizePathTraversal verifies that saveSignature cannot be
// made to write outside of the signature cache directory, even when the
// docker-reference tag it derives the destination filename from is
// attacker/signer-controlled.
//
// imge.ParseRef does not validate its "tag" component against the Docker
// reference grammar, so a docker-reference such as "repo:../pwned" yields Tag
// == "../pwned" verbatim. Without sanitization, joining that into a file path
// lets a maliciously (or accidentally) signed release payload write its cache
// signature one directory level above the intended <workingDir>/signatures/
// cache dir - i.e. directly into workingDir, where it could collide
// with/overwrite other oc-mirror state (CWE-22). A tag with more "../"
// segments could escape arbitrarily further up the filesystem the same way.
func TestSaveSignatureSanitizePathTraversal(t *testing.T) {
	log := clog.New("trace")
	...
	// The file must never be written directly into workingDir (i.e. one level
	// above the signature cache dir, which is what the unsanitized "../pwned"
	// tag targets).
	...
	// It must instead land safely inside the signature cache directory, with the traversal sequence stripped out of the filename entirely.
