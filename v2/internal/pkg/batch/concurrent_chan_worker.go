	...
	go func() {
		...
		for _, img := range collectorSchema.AllImages {
			...
			go func(ctx context.Context, semaphore chan struct{}, results chan<- GoroutineResult, spinner *mpb.Bar) {
				...
				// OCPBUGS-44448
				if opts.IsDelete() {
					if isAlreadyDeleted(img.Origin, alreadyDeleted) {
...
// OCPBUGS-44448
func isAlreadyDeleted(image string, alreadyDeleted onlyKeyMap) bool {
	if len(alreadyDeleted) > 0 && alreadyDeleted.Has(image) {
		return true
	}
