...
func NewDeleteCommand(log clog.PluggableLoggerInterface) *cobra.Command {
	...
	cmd.Flags().UintVar(&opts.ParallelLayerImages, "parallel-layers", maxParallelLayerDownloads, "Indicates the number of image layers deleted in parallel")
	cmd.Flags().UintVar(&opts.ParallelImages, "parallel-images", maxParallelImageDownloads, "Indicates the number of images deleted in parallel")
	// nolint: errcheck
