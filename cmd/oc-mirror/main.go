...
func main() {
	...
	case useV1:
		if err := runOcMirrorV1(os.Args); err != nil {
			klog.Error("failed to run oc-mirror: %s", err.Error())
...
func runOcMirrorV1(args []string) error {
	...
	if err != nil && strings.Contains(err.Error(), "permission denied") {
		...
		klog.Error("The tmp dir %q might be mounted as `noexec`. Please set TMPDIR to a filesystem with exec permissions.", tmpdir)
	}
